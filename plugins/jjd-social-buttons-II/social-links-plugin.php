...
add_action('admin_enqueue_scripts', 'jjd_social_assets_admin');
function jjd_social_assets_admin(){
	wp_enqueue_media();
	...
	//wp_enqueue_style('jjd_styles1',plugin_dir_url(__FILE__).'/css/jjd_social_style.css');
	//wp_enqueue_style('jjd_styles',plugins_url( 'css/', __FILE__ ) . 'jjd_social_style.css');
}
add_action('wp_enqueue_scripts', 'jjd_social_assets_fe');
function jjd_social_assets_fe(){
	//wp_enqueue_media();
	//wp_enqueue_script('jjd_scripts',plugin_dir_url(__FILE__).'/js/jjd_social_script.js', array('jquery', 'media-upload', 'media-views'), '1.0', true);
...
function jjd_social_buttons_function($content){
	if(is_single() || is_home() || is_front_page()):
		...
		$fb_appid = $jjd_social_plugin_options_arr['jjd_fb_appid'];
...
function jjd_social_register_settings(){
	//global $jjd_social_plugin_options_arr;
	//register_setting('jjd-settings-group', 'jjd_fb_appid');
	register_setting('jjd-settings-group', 'jjd_social_plugin_options');
}
	...
	<div class="wrap">
		...
		<form method="post" action="options.php">
			...
			<p>
				...
				<?php echo '<input type="text" size="20" maxlength ="45" name="jjd_social_plugin_options[jjd_fb_appid]" value="'.esc_attr($jjd_fb_appid).'" />'; ?>
			</p>
			<?php echo '<p id="img_container"><img src="'.esc_attr($jjd_img_uri).'" alt="" /></p>'; ?>
			<p>
				<?php echo '<input type="text" size="40" maxlength ="45" class="jjd_default_photo" name="jjd_social_plugin_options[jjd_img_uri]" value="'.esc_attr($jjd_img_uri).'" />'; ?>
